...
void WriteToFile2(Train*& trains) {
  ...
  if (outFile) {
    for (int i = 0; i < size2; ++i) {
      outFile << trains[i].Date << '|' << trains[i].Destination << '|' << trains[i].number.Places << std::endl;
    }
    outFile.close();
  }
  else {
...
void ReadFromFile2(Train*& trains) {
  ...
  if (inFile) {
    ...
    for (int i = 0; i < size2; ++i) {
      ...
      trains[i].Destination = line.substr(pos1 + 1, pos2 - pos1 - 1);
      trains[i].number.Places = std::stoi(line.substr(pos2 + 1));
    }
    inFile.close();
  }
  else {
